...
corresponding `int` key. As the number of elements in a vec can change at runtime, a vec type declaration does _not_ include an element count.
...
`$colorsVec` has an implicit key type of `int`, and an explicit value type of `string`. The vec is created and initialized using a _vec-literal_,
...
Elements _cannot_ be removed from a vec.
...
dict can change at runtime, a dict type declaration does _not_ include an element count. Consider the following class properties:
...
A dict has an explicit key type, as well as a value type. In this example, the dict is created and initialized using a _dict-literal_, which
...
Note, carefully, that although `$colorsVect` and `$colorsDict` result in collections of `int`/`string` pairs, these variables do _not_ have the
...
The order of the elements in the map is the order in which the elements were inserted into the dict. An element is said to _exist_ once it has
been inserted into the dict with a corresponding key. A dict is _extended_ by initializing a previously non-existent element using a new key.
...
can change at runtime, a keyset type declaration does _not_ include an element count. Consider the following class properties:
...
`$colorsSet` has a key and value type of `string`. The keyset is created and initialized using a _keyset-literal_, which has the general
