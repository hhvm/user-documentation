...
`getChildrenOfType<T as XHPChild>(): vec<T>` | Returns the XHP object's children that are instances of the specified class/interface.
...
`getFirstChildOfType<T as XHPChild>(): ?T` | Returns the first of XHP object's children that is an instance of the specified class/interface, or `null` if it has no such children.
