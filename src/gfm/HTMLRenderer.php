...
class HTMLRenderer {
  public function render(RenderContext $ctx, ASTNode $node): string {
    ...
    if ($node instanceof Blocks\BlockSequence) {
      return $node->getChildren()
        |> Vec\map($$, $child ==> $this->render($ctx, $child))
        |> Str\join($$, "\n");
    }
    ...
    if ($node instanceof Blocks\ListOfItems) {
      ...
      return $node->getItems()
        |> Vec\map($$, $item ==> $this->render($ctx, $item))
        |> Str\join($$, "\n")
        |> $start."\n".$$."\n".$end."\n";
    }
    ...
    if ($node instanceof Blocks\TableExtension) {
      ...
      for ($i = 0; $i < C\count($header); ++$i) {
        ...
        $html .=
          '<th'.$alignment.'>'.
          Str\join(Vec\map($cell, $cell ==> $this->render($ctx, $cell)), '').
          "</th>\n";
      }
      $html .= '</thead>';
